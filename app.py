...
import shutil  # For robust temporary file handling
...
def generate_podcast_transcript_with_rag(topic, text=None):
    """Generates a podcast transcript using RAG (Retrieval-Augmented Generation)."""

    if not text:
        # Basic podcast template when no text is provided
        ...
        try:
            ...
            st.error(f"Error during basic podcast generation: {e}")
            return None  # Or handle the error as appropriate

    # RAG-based podcast generation with context
    try:
       ...
       # Create and run RAG chain
        ...
        st.error(f"Error generating transcript with RAG: {e}")
        return None


# --- Function for Generating Podcast with Audio ---
def generate_podcast(topic, text=None):
    """Generates a podcast transcript and converts it to audio using fal-client."""

    if text:
        st.write("\n🎙️ Generating podcast transcript based on provided text:")
        st.write("-" * 50)
    else:
    ...
    try:
        with st.spinner("Generating Transcript..."):
            ...
            if transcript_result is None:  # Handle transcript generation failure
                st.error("Failed to generate podcast transcript.")
                return {"conversation": None, "audio_url": None, "error": "Transcript generation failed"}

    except Exception as e:
        st.error(f"Error generating transcript: {e}")
        return {"conversation": None, "audio_url": None, "error": str(e)}
    ...
    try:
        with st.spinner("Generating Audio..."):
            fal_api_key = os.getenv("FAL_KEY")  # Retrieve API key from environment
            if not fal_api_key:
            ...
            result = fal_client.subscribe(
                ...
                api_key=fal_api_key  # Pass the API key
            )
    ...
    if page == "Topic-Based Podcast":
        ...
        if st.button("Generate Podcast"):
            if topic:
                ...
                elif podcast_data and podcast_data["error"]:
                    st.error(f"Podcast generation failed: {podcast_data['error']}")
            else:
            ...
            if uploaded_file and podcast_title:
                try:
                    with st.spinner('Loading and processing document...'):
                        ...
                        if text is None: # Error already handled within extract_text
